...
bot: TeleBot = telebot.TeleBot("1387989780:AAElHMeKEgKUxAu9pvZtEXFWZnlglpX5gZE")
...
def help(message):
    ...
    msg = bot.send_message(message.chat.id, "1.Калькулятор - поможет с вычислением любых чисел....  " +
                           ...
                           " 6. С помощью шестой кнопки вы можите поделиться этим ботом в телеграмм"
                           "7.Погода",
                           reply_markup=markup)
...
def message(message):
    if message.text == 'Привет':
        ...
        bot.register_next_step_handler(msg, process_111)

def process_111(message):
        try:
